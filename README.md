...
**Tagline:** _We don't make your prompt better. We make it machine-readable._
...
You can. Open Claude and type _"rewrite my prompt with `<context>`, `<constraints>`, `<rules>`, `<task>` XML tags and strip the filler."_ It works.
...
- **Cache-aware structure.** Anthropic and OpenAI cache _prefixes_. A canonical, predictable IR skeleton (always `<context>` → `<constraints>` → `<rules>` → `<task>`) lets you place a stable system-prompt wrapper in front of every API call and reliably hit the cached prefix path. The IR varies; the wrapper around it doesn't — and that's where the ~90% discount lives.
...
| ---------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| PromptPerfect _(sunsetting)_       | General prompt rewriting                                       | Modern, dev-first alternative; free tier; structured IR output instead of prose.                                   |
...
- **"Why" tooltips** on every refinement — hover over a removed segment to see: _"Removed 45 tokens of politeness ('I was wondering if…') and replaced with XML tagging to prevent context rot."_
...
_A beautiful, working UI that already feels like a compiler, with all client-side metrics live._
...
_Real refinement, real metrics, structured output, "Why" explanations._
...
_Turn the compiler into something developers trust and share._
...
- **Dollars saved per prompt are small.** A heavy user might save ~$5–10/month on API costs. The compounding metric is _time saved from getting it right the first time_, not the API bill. The dollar meter exists because it's a satisfying signal.
...
- **Tight, high-density prompts may compile to a _larger_ IR and inflate downstream output.** Empirical testing across four prompt classes against Gemini 2.5 Flash:

  | Source class                                       | Density  | Source → IR   | Downstream output Δ | Verdict        |
  | -------------------------------------------------- | -------- | ------------- | ------------------- | -------------- |
  | Filler-heavy ("hey could you maybe…")              | ~46%     | 39 → 26 tok   | **−57%**            | 🟢 big win     |
  | Casual question ("hey what database…")             | ~60–70%  | 42 → 88 tok   | −12%                | 🟡 small win   |
  | Rambling spec                                      | mid      | 114 → 124 tok | −21%                | 🟡 modest win  |
  | Tight imperative ("Build X. Validate Y. Never Z.") | **~94%** | 37 → 81 tok   | **+45%**            | 🔴 active loss |

  The density score on the Source panel is the signal. Above ~85% density at ≥10 tokens, the UI shows an inline hint suggesting you skip the compile pass — the structural scaffolding tends to outweigh the gains. **For everything else, the IR helps.**

### Complementary Tools

Prompt-IR shapes _input_ — the prompt the model sees. A few projects we're aware of attack the other end:

- **[Caveman](https://github.com/JuliusBrussee/caveman)** — a Claude Code skill that compresses _output_ tokens by instructing the model to respond in terse, fragment-based prose (~65% claimed). Different layer of the pipeline; the two compose well in workflows where you control both ends.
